...
Work_with_exFAT::Work_with_exFAT(BYTE * BufferRecord, HANDLE fileHandle)
{
	ExFAT_BootRecord * pBootRecord = (ExFAT_BootRecord *)BufferRecord;
	memcpy(OEM_Name, pBootRecord->OEM_Name, sizeof(OEM_Name));
	BytesPerSector = pow(2, pBootRecord->SectorFactor);
	SectorsPerCluster = pow(2, pBootRecord->ClusterFactor);
	BytesPerCluster = BytesPerSector * SectorsPerCluster;
	TotalClusters = pBootRecord->SectorsTotal * SectorsPerCluster;
	TotalSectors = pBootRecord->SectorsTotal;
	FileHandle = fileHandle;
}

bool Work_with_exFAT::ReadClusters(ULONGLONG startCluster, DWORD numberOfClusters, BYTE * outBuffer)
{
	DWORD bytesToRead;
	DWORD bytesRead;
...
Work_with_exFAT::~Work_with_exFAT()
{
